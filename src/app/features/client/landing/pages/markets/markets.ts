...
import { MarketsService } from './services/market.service';
import { CryptoPair } from '../../../../../core/models/market.model';
import { MarketsTableComponent } from './components/markets-table/markets-table';
...
export class MarketsComponent implements OnDestroy, OnInit {
  ...
  loadMarkets(): void {
    this.marketsService.getCryptoPairs().subscribe({
      next: () => {
        if (this.showVisualization() && this.isBrowser) {
          setTimeout(() => this.drawAllCharts(), 1000);
        }
  ...
  startRealTimeUpdates(): void {
    ...
    timer(0, 10000) // First update after 5s, then every 5s
      .pipe(
